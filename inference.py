...
def run_inference_old(num_examples):
	"""
	Load saved model and run inference on random images selected from dataset
	"""
	...
	with tf.Graph().as_default(), tf.Session() as sess:
		...
		print('Running inference on %d random images' % num_examples)
		all_image_files = list(train.keys())
		image_files = np.random.choice(all_image_files, num_examples, False)
		...
		for image_file in image_files:
			...
			print('Inference took %.1f sec (%.2f fps)' % ((time.time() - t0)/1000, 1/(time.time() - t0)))
			...
			print('Inference + NMS took %.4f sec (%.2f fps)' % (time.time() - t0, 1/(time.time() - t0)))

			# For drawing, convert image to RGB
			image = cv2.cvtColor(image, cv2.COLOR_GRAY2RGB)

			# Draw boxes on image and annotate w/ predicted class and confidence
			for box in boxes:
				box_coords = box[:4]
				cls = box[4]
				cls_prob = box[5]
				...
				label_str = '%s %.2f' % (sign_map[int(cls)][0], cls_prob)
				image = cv2.putText(image, label_str, (box_coords[0], box_coords[1]+10), 0, 0.2, (0,255,0), 1, cv2.LINE_AA)



			# Draw final image
			print('Displaying %s' % image_file)
			plt.imshow(image)
			plt.show()
